...
(defun mark-by-direction (is-forward buffer)
  "Mark an isearch match with a cursor either before or after the current position in the BUFFER.
  IS-FORWARD "
  (alexandria:when-let ((str (or (buffer-value buffer 'isearch-redisplay-string)
                                 (and (boundp '*isearch-string*)
                                      *isearch-string*))))
    (let* ((cur-p (buffer-point buffer))
           (string-length (length str)))
      (declare (type lem-core:point cur-p)
               (type integer string-length)
               (optimize (speed 3) (safety 2)))
      (destructuring-bind (start end)
          (determine-start-end-current-search cur-p str is-forward)
        (let* ((pre-offset (cond
               ...
               (c (first sorted-cursors)))
          (declare (type integer pre-offset post-offset)
                   (type (list lem-core:cursor) cursors)
                   (type (list lem-core:cursor) sorted-cursors)
                   (type lem-core:cursor c))
          (with-point ((point c))
    ...
    (alexandria:when-let ((str (or (buffer-value buffer 'isearch-redisplay-string)
      ...
      (let* ((string-length (length str))
        ...
        (with-point ((p (buffer-start-point buffer)))
          (loop :while (funcall search-fn p str)
                :do (with-point ((start p))
                      (funcall search-back-fn start str)
